...
// import type { FeatureCollection, Polygon, LineString, Feature, Position } from 'geojson';
...
// // ---- Map helpers ----
// const ENV_STYLE = import.meta.env.VITE_MAP_STYLE
...
// // small seeded RNG for stable “random” per subarea
...
//   const widthKm = 2.5 / 1000;  // 2.5 m
//   const depthKm = 5 / 1000;    // 5 m

//   const pick = [...filtered].sort((a, b) => a.code.localeCompare(b.code)).slice(0, 6);
//   const slots: Feature<Polygon>[] = [];

//   for (let i = 0; i < pick.length; i++) {
//     const s = pick[i];
//     const dKm = spacingKm * (i + 1);
//     const mid = along(road, dKm, { units: 'kilometers' }); // Feature<Point>

//     // Build oriented rectangle around `mid`
...
//   return featureCollection<LineString | Polygon>([road, ...slots]);
// }
...
//   // ---------- MAP PREVIEW (random road + 6 slots via Turf) ----------
//   const mapRef = useRef<MLMap | null>(null);
//   const mapElRef = useRef<HTMLDivElement | null>(null);
//   const [styleReady, setStyleReady] = useState(false);

//   const previewFC = useMemo<FeatureCollection<LineString | Polygon>>(
//     () => buildPreviewFeatures((subareaId ?? 'kashiwa') + '::' + filtered.length, filtered),
//     [subareaId, filtered]
//   );
...
//     const onLoad = () => { setStyleReady(true); map.resize(); setTimeout(() => map.resize(), 0); };
//     const onStyleData = () => setStyleReady(isStyleLoadedSafe(map));
//     map.on('load', onLoad);
//     map.on('styledata', onStyleData);

//     // log & fallback if style fails
//     map.on('error', (e) => {
...
//       <div className="relative w-full h-64 sm:h-80 rounded-2xl overflow-hidden border mb-4">
//         <div ref={mapElRef} className="absolute inset-0 z-50 w-full" />
//         {/* Legend */}
